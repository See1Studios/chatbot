...
def search_sessions(
    ...
    results = []
    session_files = sorted(SESSIONS_DIR.glob("*/meta.json"), key=lambda p: p.stat().st_mtime, reverse=True)
...
def inspect_session(session_id: str) -> Optional[dict]:
    target = SESSIONS_DIR / session_id / "meta.json"
    if not target.exists():
        candidates = list(SESSIONS_DIR.glob(f"*{session_id}*/meta.json"))
        if candidates:
            target = candidates[0]
        else:
            return None
